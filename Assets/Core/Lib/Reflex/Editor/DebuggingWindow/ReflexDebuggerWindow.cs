...
namespace Reflex.Editor.DebuggingWindow
{
    public class ReflexDebuggerWindow : EditorWindow
    {
        ...
        private static FieldInfo _resolversField;

        public ReflexDebuggerWindow() => _resolversField = typeof(Context).GetField("_resolvers", BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        private void BuildDataRecursively(MyTreeElement parent, Context context)
        {
            if (context == null)
            {
                return;
            ...
            var child = new MyTreeElement(context.Name, parent.Depth + 1, ++_id, ContainerIcon, () => string.Empty);
            parent.Children.Add(child);
            child.Parent = parent;

            foreach (var pair in (Dictionary<Type, IResolver>)_resolversField.GetValue(context))
            {
        ...
        private IList<MyTreeElement> GetData()
        {
            ...
            BuildDataRecursively(root, ContextTree.Root);
